...
int main(void)
{
	...
	display(m1); 
    display(m2);
    printf("   RESULT OF STRASSEN'S  \n");
    clock_t start1,end1,start2,end2;
	start1 = clock();
	display(multiply(m1, m2));
	end1 = clock();
	printf("\n TIME OF EXECUTION  OF STRASSEN'S ALGO : %fl\n",((double)end1 - (double)start1)/CLOCKS_PER_SEC);
	printf("\n   RESULT OF NAIVE METHOD  \n");
	start2 = clock();
	display(naive_multiply(m1, m2));
	end2 = clock();
	printf("\n TIME OF EXECUTION  OF NAIVE METHOD: %fl",((double)end2 - (double)start2)/CLOCKS_PER_SEC); 
